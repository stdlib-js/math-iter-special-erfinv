...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterErfinv, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	values = [ 0.0, 0.5, 1.0 ];
	it = iterErfinv( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the inverse error function of each iterated value', function test( t ) {
	var expected;
	var it;
	var N;
	var x;
	...
	N = 101;
	it = iterErfinv( linspace( -1.0, 1.0, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( -1.0, 1.0, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = erfinv( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the inverse error function of each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': erfinv( values[ 0 ] ),
			'done': false
		},
		{
			'value': erfinv( values[ 1 ] ),
			'done': false
		},
		{
			'value': erfinv( values[ 2 ] ),
			'done': false
		},
		{
			'value': erfinv( values[ 3 ] ),
			'done': true
		}
	];
